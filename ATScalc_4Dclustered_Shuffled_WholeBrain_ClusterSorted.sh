...
MINBETA=0.01
CLUSTERSIZE=5
MYMASK=${PRJDIR}/Masks/Cingm_3mm.nii.gz
ROIMASK='Cingm'
...
do
	...
	pwd
	BRAINMASK=${CRDIR}/${SBJID%%_*}/${SBJID##*_}/mask_MNI.nii.gz
		...
		do
			...
			if [ $MINBETA > 0 ]
				then
					...
					3dcalc -a THC_beta_pos_${DATASET} -b THC_beta_neg_${DATASET} -expr "a+b" -prefix THC_beta_${DATASET} -overwrite
  					...
  					# 3dmerge -dxyz=1 -1clust_mean 1 ${CLUSTERSIZE} -doall -prefix THC_beta_${DATASET} THAmp_beta_${DATASET} -overwrite
					# 3dClusterize -inset THAmp_beta_${DATASET} -bisided -1 1 -NN 1 -ithr 0 -clust_nvox ${CLUSTERSIZE} -pref_map THC_beta_${DATASET} -overwrite
				else
							# \\ Apply minimum cluster size in beta estimates from 3dPFM
					...
					3dClusterize -inset beta_${DATASET:0:5}_EqualShuffle_4D_clust.nii.gz -bisided -1 1 -clust_nvox ${CLUSTERSIZE} -pref_map THC_beta_${DATASET:0:5}_EqualShuffle_4D_clust.nii.gz -overwrite
			fi
			...
			echo -e "\033[1mComputing Activation Time Series POSITIVE for ${DATASET}\033[0m"
 			3dcalc -a ${ATS_DATASET} -expr 'posval(a)' -prefix temp_pos -overwrite
  			3dcalc -a ${ATS_DATASET} -expr 'ispositive(a)' -prefix temp_pos_bool -overwrite
  				# # \\ Whole brain mask
  			3dmaskave -quiet -mask ${BRAINMASK} -sum temp_pos+tlrc. > ATSsum_pos.${ATS_DATASET:0:56}_WholeBrain.1D
  			3dmaskave -quiet -mask ${BRAINMASK} -sum temp_pos_bool+tlrc. > ATSnum_pos.${ATS_DATASET:0:56}_WholeBrain.1D
			...
			echo -e "\033[1mComputing Activation Time Series NEGATIVE for ${DATASET}\033[0m"
  			...
  			3dcalc -a ${ATS_DATASET} -expr 'ispositive(-1*a)' -prefix temp_neg_bool -overwrite
  				# # \\ Whole brain mask
  			3dmaskave -quiet -mask ${BRAINMASK} -sum temp_neg+tlrc. > ATSsum_neg.${ATS_DATASET:0:56}_WholeBrain.1D
  			3dmaskave -quiet -mask ${BRAINMASK} -sum temp_neg_bool+tlrc. > ATSnum_neg.${ATS_DATASET:0:56}_WholeBrain.1D
			...
			echo -e "\033[1mComputing Activation Time Series ABSOLUTE for ${DATASET}\033[0m"
  			...
  			3dcalc -a ${ATS_DATASET} -expr 'bool(abs(a))' -prefix temp_abs_bool -overwrite
  				# # \\ Whole brain mask
  			3dmaskave -quiet -mask ${BRAINMASK} -sum temp_abs+tlrc. > ATSsum_abs.${ATS_DATASET:0:56}_WholeBrain.1D
  			3dmaskave -quiet -mask ${BRAINMASK} -sum temp_abs_bool+tlrc. > ATSnum_abs.${ATS_DATASET:0:56}_WholeBrain.1D
